...
ggplot(weights, aes(x = weight)) +
  geom_histogram(bins = 50, color = "black", alpha = 0.1, aes(y = after_stat(density))) +
...
To demonstrate the different approaches, we use the dataset on the weights. The
dataset contains the weight for all students enrolled in a particular school. 
...
We will work out a case study using the **air quality** dataset.
...
We explore the missingness in the dataset. We can visualize the missing data
patterns using the **plot_pattern()** function from the **ggmice** package. We
see there is a total of 44 missing values. There are 37 missing values for the
variable **Ozone** and seven missing values for the variable **Solar.R**.

```{r}
ggmice::plot_pattern(data = airquality, square = F, rotate = T)
```

We can also create an influx-outflux plot. The **influx** of a variable shows
how well its missing values connect to the observed values of other variables.
The **outflux** of a variable shows how well its observed values connect to the
missing values of other variables.

```{r}
ggmice::plot_flux(data = airquality, label = F)
```

### Relations

The variables with missing values are **Ozone** and **Solar.R**. We can further
investigate their relationship with the other variables to establish appropriate
imputation models.

First, we can also plot the correlations between the variables. These are the
Pearson correlation coefficients, which is a statistical measure to quantify
direction and strength of a linear relationship between two variables.
Therefore, it could also be beneficial to obtain Spearman rank correlation
coefficients, for which we would rather use the **ggcorrplot** package.

```{r}
# Create a correlation (Pearson) matrix
ggmice::plot_corr(data = airquality, label = T, diagonal = T, rotate = T)
```

Let us now use **ggcorrplot** to create the same correlation matrix.

```{r}
# Create a correlation (Pearson) matrix
p_corr <- cor(x = airquality, use = "pairwise.complete.obs", method = "pearson")
ggcorrplot::ggcorrplot(corr = p_corr, lab = T, digits = 3)
```

Because we use base R and **ggcorrplot** we have more flexibility and can also
use the Spearman rank correlation coefficients.

```{r}
# Create a correlation (Spearman) matrix
s_corr <- cor(x = airquality, use = "pairwise.complete.obs", method = "spearman")
ggcorrplot::ggcorrplot(corr = s_corr, lab = T, digits = 3)
```

Let us look at the relationship of **Ozone** with the other variables. There
appears to be a relationship between **Ozone** and the variables **Wind** and
**Temp**.

```{r}
ggmice::ggmice(data = airquality, aes(Ozone, Solar.R)) +
  geom_point() +
  geom_smooth(formula = "y ~ x", method = "loess", se = F, color = "black")
```

```{r}
ggmice::ggmice(data = airquality, aes(Ozone, Wind)) +
  geom_point() +
  geom_smooth(formula = "y ~ x", method = "loess", se = F, color = "black")
```

```{r}
ggmice::ggmice(data = airquality, aes(Ozone, Temp)) +
  geom_point() +
  geom_smooth(formula = "y ~ x", method = "loess", se = F, color = "black")
```

```{r}
ggmice::ggmice(data = airquality, aes(Ozone, Month)) +
  geom_point() +
  geom_smooth(formula = "y ~ x", method = "loess", se = F, color = "black")
```

```{r}
ggmice::ggmice(data = airquality, aes(Ozone, Day)) +
  geom_point() +
  geom_smooth(formula = "y ~ x", method = "loess", se = F, color = "black")
```

Let us look at the relationship of **Solar.R** with the other variables.

```{r}
ggmice::ggmice(data = airquality, aes(Solar.R, Ozone)) +
  geom_point() +
  geom_smooth(formula = "y ~ x", method = "loess", se = F, color = "black")
```

```{r}
ggmice::ggmice(data = airquality, aes(Solar.R, Wind)) +
  geom_point() +
  geom_smooth(formula = "y ~ x", method = "loess", se = F, color = "black")
```

```{r}
ggmice::ggmice(data = airquality, aes(Solar.R, Temp)) +
  geom_point() +
  geom_smooth(formula = "y ~ x", method = "loess", se = F, color = "black")
```

```{r}
ggmice::ggmice(data = airquality, aes(Solar.R, Month)) +
  geom_point() +
  geom_smooth(formula = "y ~ x", method = "loess", se = F, color = "black")
```

```{r}
ggmice::ggmice(data = airquality, aes(Solar.R, Day)) +
  geom_point() +
  geom_smooth(formula = "y ~ x", method = "loess", se = F, color = "black")
```
...
We wil now impute the data.

```{r}
# Impute missing values using MICE
imp <- mice::mice(data = airquality, printFlag = F)
```

Let us inspect the imputed data. The following figures show one blue dot for
every observed value and one red dot for every imputed value. So, if we choose
to obtain five imputed datasets, then there will be five imputed values for
every missing value.

```{r}
ggmice::ggmice(data = imp, aes(.imp, Ozone)) +
...
```{r}
figs <- purrr::map(c("Solar.R", "Wind", "Temp"), ~ {
  
  fig_1 <- ggmice::ggmice(data = airquality, aes(Ozone, .data[[.x]])) +
    geom_point() +
    geom_smooth(formula = "y ~ x", method = "loess", se = F, color = "black")
  
  fig_2 <- ggmice::ggmice(data = imp, aes(Ozone, .data[[.x]])) +
    geom_point() +
    geom_smooth(formula = "y ~ x", method = "loess", se = F, color = "black")
...
```{r}
ggmice::ggmice(data = imp, aes(.imp, Solar.R)) +
  geom_jitter(height = 0, width = 0.25) +
  geom_boxplot(width = 0.5, size = 1, alpha = 0.75, outlier.shape = NA) +
  labs(x = "Imputation number")
```
...
```{r}
figs <- purrr::map(c("Ozone", "Wind", "Temp"), ~ {
  
  fig_1 <- ggmice::ggmice(data = airquality, aes(Solar.R, .data[[.x]])) +
    geom_point() +
    geom_smooth(formula = "y ~ x", method = "loess", se = F, color = "black")
  
  fig_2 <- ggmice::ggmice(data = imp, aes(Solar.R, .data[[.x]])) +
    geom_point() +
    geom_smooth(formula = "y ~ x", method = "loess", se = F, color = "black")
